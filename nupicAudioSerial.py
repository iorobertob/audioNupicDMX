...
os - to have priority
serial - to send usb serial data for the DMX bus.
time - there are delays. 
...
class AudioStream:
  ...
  def __init__(self):
    ...
    baud rate = 250k
    """########################################################################
    self.DmxBufferToSend = [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00]

    serlf.ser = serial.Serial('port', 250000)
    #configure 2 stops bits for DMX spec
    ser.setStopbits(2)
    
    #512 channels + start byte (0's)
    DMX_BUFFER_SIZE = 513
     
    #Fill the reste of the buffer with 0's
    self.DmxBufferToSend.extend([0]*(DMX_BUFFER_SIZE - len(DmxBufferToSend) ))
  ...
  def processAudio (self):
    ...
    """
    Moving average - lowpass filter on the anomaly score, slow it down.
    """
    self.anomalyprev = (anomaly + self.anomalyprev)/2
    print self.vis.hashtagAnomaly(self.anomalyprev)
    print self.anomalyprev
    self.anomalyprev = anomaly
    

    """
    Send anomaly score over Serial,  DMX
    1st argument - channel
    2nd argument - value
    """    
    self.sendDMX(1, chr(int(anomaly*255)))
  ...
  def sendDMX(self, channel, value):
    """
    Send DMX packets, it sends a 513 bytes array every time. 
    """

    #assign value to channel, could be taken out of this function. 
    DmxBufferToSend[channel] = value;
    
    #send break condition
    self.ser.sendBreak(0.000001)
    #Write all Dmx Buffer
    self.ser.write(DmxBufferToSend)
    #Clean the output
    self.ser.flushOutput()
    #Sleep 1 ms before to attempt other message
    time.sleep(0.001)
